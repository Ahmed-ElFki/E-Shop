...
const { avatarF, avatarM } = require("./avatar");

const registerUser = async (req, res) => {
  const { error } = JoiUserRegisterValidation(req.body);
  if (!error) {
    const emailExists = await userModel.findOne({ email: req.body.email });
    if (emailExists) res.send({ message: "Email already used" });
    else {
      ...
      const userObject = new userModel({
        ...
        avatar: req.body.gender === "Male" ? avatarM : avatarF,
...
const loginUser = async (req, res) => {
  ...
  if (!error) {
    ...
    if (!userExists) res.send({ message: "Email/Password wrong" });
    else {
      const userPwd = userExists.password;
      const validatePwd = await bcrypt.compare(req.body.password, userPwd);

      if (!validatePwd) res.send({ message: "Email/Password wrong" });
      else {
